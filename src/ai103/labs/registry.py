...
def default_registry(root: Path) -> dict[str, LabSpec]:
    return {
        ...
        "LAB-MEDIA-GENERATION": LabSpec(
            lab_id="LAB-MEDIA-GENERATION",
            title="Image generation, editing, video simulation, and media policy fixture",
            objective_ids=("GA-03", "PM-04"),
            fixture_path=root / "fixtures" / "azure-responses" / "LAB-MEDIA-GENERATION.offline.json",
            required_response_shapes=("media_generation",),
            checks=(
                LabCheckSpec(
                    id="image-generation-simulated",
                    response="media_generation",
                    path=("image_generation", "offline_simulation"),
                    expected=True,
                    evidence_template="Image generation offline simulation status was {value}.",
                ),
                LabCheckSpec(
                    id="editing-mask-workflow",
                    response="media_generation",
                    path=("image_editing", "mask_workflow"),
                    expected=True,
                    evidence_template="Image editing mask workflow status was {value}.",
                ),
                LabCheckSpec(
                    id="video-preview-simulated",
                    response="media_generation",
                    path=("video_generation", "preview_simulation"),
                    expected=True,
                    evidence_template="Video preview simulation status was {value}.",
                ),
                LabCheckSpec(
                    id="media-policy-labeled",
                    response="media_generation",
                    path=("policy", "generated_media_labeled"),
                    expected=True,
                    evidence_template="Generated media labeling status was {value}.",
                ),
            ),
        ),
        "LAB-SPEECH-TRANSLATION": LabSpec(
            lab_id="LAB-SPEECH-TRANSLATION",
            title="Speech-to-text, text-to-speech, and translation fixture",
            objective_ids=("TA-02", "TA-03", "TA-04", "PM-04"),
            fixture_path=root / "fixtures" / "azure-responses" / "LAB-SPEECH-TRANSLATION.offline.json",
            required_response_shapes=("speech",),
            checks=(
                LabCheckSpec(
                    id="speech-to-text",
                    response="speech",
                    path=("speech_to_text", "transcript"),
                    expected="AI-103 speech lab synthetic sample.",
                    evidence_template="Speech-to-text transcript was {value}.",
                ),
                LabCheckSpec(
                    id="text-to-speech",
                    response="speech",
                    path=("text_to_speech", "synthesis_labeled"),
                    expected=True,
                    evidence_template="Text-to-speech synthesis labeling status was {value}.",
                ),
                LabCheckSpec(
                    id="translation",
                    response="speech",
                    ...
                    expected="es",
                    evidence_template="Translation target language was {value}.",
                ),
                LabCheckSpec(
                    id="audio-consent-license",
                    response="speech",
                    path=("consent_license", "valid"),
                    expected=True,
                    evidence_template="Audio consent and license status was {value}.",
                ),
            ),
        ),
        "LAB-GOVERNANCE-MONITORING": LabSpec(
            lab_id="LAB-GOVERNANCE-MONITORING",
            title="Responsible AI filters, approval controls, monitoring, quota, cost, and health fixture",
            objective_ids=("PM-03", "PM-04", "PM-05", "GA-04"),
            fixture_path=root / "fixtures" / "azure-responses" / "LAB-GOVERNANCE-MONITORING.offline.json",
            required_response_shapes=("governance",),
            checks=(
                LabCheckSpec(
                    id="unsafe-behavior-blocked",
                    response="governance",
                    path=("responsible_ai", "unsafe_request_blocked"),
                    expected=True,
                    evidence_template="Unsafe behavior block status was {value}.",
                ),
                LabCheckSpec(
                    id="approval-controls",
                    response="governance",
                    path=("approval_controls", "billable_action_requires_approval"),
                    expected=True,
                    evidence_template="Billable approval control status was {value}.",
                ),
                LabCheckSpec(
                    id="monitoring-trace",
                    response="governance",
                    path=("monitoring", "trace_captured"),
                    expected=True,
                    evidence_template="Monitoring trace capture status was {value}.",
                ),
                LabCheckSpec(
                    id="quota-cost-health",
                    response="governance",
                    path=("operations", "quota_cost_health_checked"),
                    expected=True,
                    evidence_template="Quota, cost, and health inspection status was {value}.",
                ),
            ),
        ),
