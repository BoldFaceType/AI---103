---
{
  "schema_version": 1,
  "lesson_id": "PM-04",
  "title": "Configure deployments and CI/CD delivery",
  "competency_ids": [
    "PM-07",
    "PM-08"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/PM-04-deployment-cicd.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/pm/PM-04.json"
  ],
...
# Configure deployments and CI/CD delivery
...
Scenario focus: Plan model and agent deployment promotion through source-controlled environments. This lesson specifically practices deployment-cicd. The learner must include rejecting manual portal-only changes for repeatable project delivery. The target competencies are PM-07, PM-08.
...
A product team asks for an Azure AI capability to support the scenario: Plan model and agent deployment promotion through source-controlled environments. The learner must choose the service or governance pattern, name the evidence artifact, and document why the rejected option fails the workload constraints.
...
4. Reject one unsuitable option: rejecting manual portal-only changes for repeatable project delivery.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/pm/PM-04.json"
}
```
