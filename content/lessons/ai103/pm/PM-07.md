---
{
  "schema_version": 1,
  "lesson_id": "PM-07",
  "title": "Secure identity networking RBAC and policy boundaries",
  "competency_ids": [
    "PM-12"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/PM-07-least-privilege.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/pm/PM-07.json"
  ],
...
# Secure identity networking RBAC and policy boundaries
...
Scenario focus: Apply keyless authentication, private network design, and least-privilege RBAC to an AI workload. This lesson specifically practices least-privilege. The learner must include rejecting broad contributor access when managed identity and scoped roles are enough. The target competencies are PM-12.
...
A product team asks for an Azure AI capability to support the scenario: Apply keyless authentication, private network design, and least-privilege RBAC to an AI workload. The learner must choose the service or governance pattern, name the evidence artifact, and document why the rejected option fails the workload constraints.
...
4. Reject one unsuitable option: rejecting broad contributor access when managed identity and scoped roles are enough.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/pm/PM-07.json"
}
```
