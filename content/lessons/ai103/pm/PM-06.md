---
{
  "schema_version": 1,
  "lesson_id": "PM-06",
  "title": "Monitor quality drift safety ingestion and relevance",
  "competency_ids": [
    "PM-10",
    "PM-11"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/PM-06-observability.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/pm/PM-06.json"
  ],
...
# Monitor quality drift safety ingestion and relevance
...
Scenario focus: Plan telemetry for model quality, safety events, drift, ingestion health, and search relevance. This lesson specifically practices observability. The learner must include rejecting a launch when traces show retrieval relevance is below threshold. The target competencies are PM-10, PM-11.
...
A product team asks for an Azure AI capability to support the scenario: Plan telemetry for model quality, safety events, drift, ingestion health, and search relevance. The learner must choose the service or governance pattern, name the evidence artifact, and document why the rejected option fails the workload constraints.
...
4. Reject one unsuitable option: rejecting a launch when traces show retrieval relevance is below threshold.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/pm/PM-06.json"
}
```
