---
{
  "schema_version": 1,
  "lesson_id": "PM-02",
  "title": "Choose retrieval indexing memory tools and knowledge patterns",
  "competency_ids": [
    "PM-03",
    "PM-04"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/PM-02-retrieval-indexing.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/pm/PM-02.json"
  ],
...
# Choose retrieval indexing memory tools and knowledge patterns
...
Scenario focus: Decide whether knowledge should be represented as indexed documents, agent memory, tool results, or a combination. This lesson specifically practices retrieval-indexing. The learner must include rejecting long prompt stuffing when indexed retrieval is required. The target competencies are PM-03, PM-04.
...
A product team asks for an Azure AI capability to support the scenario: Decide whether knowledge should be represented as indexed documents, agent memory, tool results, or a combination. The learner must choose the service or governance pattern, name the evidence artifact, and document why the rejected option fails the workload constraints.
...
4. Reject one unsuitable option: rejecting long prompt stuffing when indexed retrieval is required.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/pm/PM-02.json"
}
```
