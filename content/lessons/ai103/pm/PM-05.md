---
{
  "schema_version": 1,
  "lesson_id": "PM-05",
  "title": "Manage quotas scale rate limits cost and budgets",
  "competency_ids": [
    "PM-09"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/PM-05-quota-cost.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/pm/PM-05.json"
  ],
...
# Manage quotas scale rate limits cost and budgets
...
Scenario focus: Design capacity, throttling behavior, and budget guardrails before scaling usage. This lesson specifically practices quota-cost. The learner must include stopping a lab plan because projected token and index costs exceed budget. The target competencies are PM-09.
...
A product team asks for an Azure AI capability to support the scenario: Design capacity, throttling behavior, and budget guardrails before scaling usage. The learner must choose the service or governance pattern, name the evidence artifact, and document why the rejected option fails the workload constraints.
...
4. Reject one unsuitable option: stopping a lab plan because projected token and index costs exceed budget.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/pm/PM-05.json"
}
```
