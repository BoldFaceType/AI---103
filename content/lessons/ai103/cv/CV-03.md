---
{
  "schema_version": 1,
  "lesson_id": "CV-03",
  "title": "Analyze visual context with multimodal models",
  "competency_ids": [
    "CV-06",
    "CV-07",
    "CV-08"
  ...
  "lab_links": [
    "scripts/labs/planned/CV-03-multimodal-understanding.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/cv/CV-03.json"
  ],
...
Scenario focus: analyze visual context, generate concise or detailed captions, and answer questions using visual evidence as grounding. The target competencies are CV-06, CV-07, CV-08.
...
A field technician submits an equipment photo and asks what visible part appears damaged; the app requires captions, visual evidence, and a refusal when the image does not support the answer. The learner must document image rights, provenance, current availability, required alt text, policy checks, and an offline fixture before live Azure use.
...
4. Reject one unsuitable option: Reject answering from world knowledge when the image does not contain the claimed evidence.
...
- Keyword focus for review: multimodal understanding, caption, visual question answering, grounding, alt-text quality.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/cv/CV-03.json"
}
```
