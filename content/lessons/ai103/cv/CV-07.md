---
{
  "schema_version": 1,
  "lesson_id": "CV-07",
  "title": "Apply responsible multimodal safety, injection, and visual policy gates",
  "competency_ids": [
    "CV-14",
    "CV-15",
    "CV-16"
  ...
  "lab_links": [
    "scripts/labs/planned/CV-07-responsible-multimodal.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/cv/CV-07.json"
  ],
...
# Apply responsible multimodal safety, injection, and visual policy gates
...
Scenario focus: classify unsafe visual content, detect indirect prompt injection embedded in images, and enforce visual policies such as watermarks, brand rules, prohibited symbols, and inappropriate content detection. The target competencies are CV-14, CV-15, CV-16.
...
A support app rejects text hidden inside an uploaded image that says to ignore prior instructions, checks generated media for watermark and brand policy requirements, and records the safety decision. The learner must document image rights, provenance, current availability, required alt text, policy checks, and an offline fixture before live Azure use.
...
4. Reject one unsuitable option: Reject following instructions extracted from an image because embedded text is untrusted visual content, not an authorized developer command.
...
- Keyword focus for review: unsafe content, indirect prompt injection embedded in images, watermark, policy, brand rules.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/cv/CV-07.json"
}
```
