---
{
  "schema_version": 1,
  "lesson_id": "CV-02",
  "title": "Edit images and videos with masks and platform controls",
  "competency_ids": [
    "CV-03",
    "CV-04",
    "CV-05"
  ...
  "lab_links": [
    "scripts/labs/planned/CV-02-editing.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/cv/CV-02.json"
  ],
...
# Edit images and videos with masks and platform controls
...
Scenario focus: configure inpainting, mask edits, prompt-driven image editing, video edits, and platform-appropriate controls. The target competencies are CV-03, CV-04, CV-05.
...
A designer edits a generated product mockup by applying a mask, requesting a background change, and recording which edit controls are supported by the current platform. The learner must document image rights, provenance, current availability, required alt text, policy checks, and an offline fixture before live Azure use.
...
4. Reject one unsuitable option: Reject a workflow that assumes every preview-only editing control is generally available in every region or deployment.
...
- Keyword focus for review: inpainting, mask edit, video editing, preview-only, current availability.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/cv/CV-02.json"
}
```
