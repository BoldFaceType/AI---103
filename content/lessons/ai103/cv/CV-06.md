---
{
  "schema_version": 1,
  "lesson_id": "CV-06",
  "title": "Interpret video segments, objects, components, and regions",
  "competency_ids": [
    "CV-11",
    "CV-13"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/CV-06-video-regions.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/cv/CV-06.json"
  ],
...
# Interpret video segments, objects, components, and regions
...
Scenario focus: process and interpret video segments while identifying objects, components, regions, and time-bounded evidence. The target competencies are CV-11, CV-13.
...
A safety review workflow samples warehouse video segments, labels visible forklift interactions, and records the frame or time range that supports each finding. The learner must document image rights, provenance, current availability, required alt text, policy checks, and an offline fixture before live Azure use.
...
4. Reject one unsuitable option: Reject a summary that reports an object or event without frame, region, or segment evidence.
...
- Keyword focus for review: video segment, object detection, region evidence, time range, offline fixture.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/cv/CV-06.json"
}
```
