---
{
  "schema_version": 1,
  "lesson_id": "IE-05",
  "title": "Connect retrieval pipelines to workflows and agent tools",
  "competency_ids": [
    "IE-05"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/IE-05-agent-retrieval-tools.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ie/IE-05.json"
  ],
...
Scenario focus: connect retrieval pipelines directly to workflows and agent retrieval tools with schema validation and safe result handling. The target competencies are IE-05.
...
An agent retrieval tool accepts a typed query, retrieves approved passages, returns citations and confidence metadata, and refuses tool execution when arguments fail validation. The learner must document search mode, OCR or analyzer evidence, enrichment output, citations, and offline fixtures before live Azure use.
...
4. Reject one unsuitable option: Reject an agent tool that can search arbitrary stores or return uncited context without schema validation.
...
- Keyword focus for review: agent retrieval tools, workflow, schema validation, citations, safe result handling.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ie/IE-05.json"
}
```
