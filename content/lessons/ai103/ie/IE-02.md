---
{
  "schema_version": 1,
  "lesson_id": "IE-02",
  "title": "Configure semantic, hybrid, and vector search for grounding",
  "competency_ids": [
    "IE-02"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/IE-02-search-modes.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ie/IE-02.json"
  ],
...
Scenario focus: configure semantic, hybrid, and vector search modes for grounded retrieval. The target competencies are IE-02.
...
A knowledge assistant compares keyword, semantic, vector, and hybrid retrieval fixtures for the same support question, then chooses the mode that best balances relevance, citation evidence, and latency. The learner must document search mode, OCR or analyzer evidence, enrichment output, citations, and offline fixtures before live Azure use.
...
4. Reject one unsuitable option: Reject one search mode as universal because grounding quality depends on corpus shape, query type, embedding quality, and ranking evidence.
...
- Keyword focus for review: semantic search, hybrid search, vector search, grounding, citations.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ie/IE-02.json"
}
```
