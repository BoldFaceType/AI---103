---
{
  "schema_version": 1,
  "lesson_id": "IE-04",
  "title": "Build RAG ingestion flows with documents and OCR",
  "competency_ids": [
    "IE-04"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/IE-04-rag-ocr.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ie/IE-04.json"
  ],
...
# Build RAG ingestion flows with documents and OCR
...
Scenario focus: configure RAG ingestion flows including document processing, OCR, chunking, metadata, and citation-ready grounding. The target competencies are IE-04.
...
A policy assistant runs a synthetic scanned policy fixture through OCR, chunking, metadata assignment, and retrieval validation before the RAG prompt receives context. The learner must document search mode, OCR or analyzer evidence, enrichment output, citations, and offline fixtures before live Azure use.
...
4. Reject one unsuitable option: Reject sending raw scanned files directly to generation when OCR, chunk quality, metadata, and citation mapping have not been verified.
...
- Keyword focus for review: RAG, OCR, chunking, metadata, document processing.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ie/IE-04.json"
}
```
