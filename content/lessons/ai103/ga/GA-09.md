---
{
  "schema_version": 1,
  "lesson_id": "GA-09",
  "title": "Monitor agents and analyze failures",
  "competency_ids": [
    "GA-12",
    "GA-15"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/GA-09-agent-operations.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ga/GA-09.json"
  ],
...
# Monitor agents and analyze failures
...
Scenario focus: Monitor deployed agents, evaluate behavior, analyze failures, and observe traces, tokens, safety signals, latency, and breakdowns. The target competencies are GA-12, GA-15.
...
An operations dashboard groups failed agent runs by tool error, retrieval miss, safety refusal, latency spike, and token-cost outlier before assigning remediation. The learner must identify the Azure component, validation gate, telemetry record, and remediation path before any live resource is used.
...
4. Reject one unsuitable option: Reject aggregate success rate alone because it hides tool failure, bad grounding, safety drift, and cost regressions.
...
- Keyword focus for review: monitor, trace, tokens, safety signals, latency, cost.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ga/GA-09.json"
}
```
