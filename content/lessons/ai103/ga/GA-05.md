---
{
  "schema_version": 1,
  "lesson_id": "GA-05",
  "title": "Use Foundry SDKs, connectors, and project connections",
  "competency_ids": [
    "GA-05",
    "GA-06"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/GA-05-foundry-connections.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ga/GA-05.json"
  ],
...
# Use Foundry SDKs, connectors, and project connections
...
Scenario focus: Integrate Foundry SDKs, connectors, and project connections into application workflows while keeping configuration, identity, and data access explicit. The target competencies are GA-05, GA-06.
...
A web app reads its Foundry project connection from configuration, uses managed identity where possible, and sends connector data only after access checks pass. The learner must identify the Azure component, validation gate, telemetry record, and remediation path before any live resource is used.
...
4. Reject one unsuitable option: Reject copying endpoint keys into source files or notebooks because credentials must be scoped and rotated outside lesson content.
...
- Keyword focus for review: Foundry SDK, connector, project connection, managed identity, configuration.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ga/GA-05.json"
}
```
