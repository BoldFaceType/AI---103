---
{
  "schema_version": 1,
  "lesson_id": "GA-03",
  "title": "Design tool-augmented workflows",
  "competency_ids": [
    "GA-03"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/GA-03-tool-workflows.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ga/GA-03.json"
  ],
...
# Design tool-augmented workflows
...
Scenario focus: Design tool-augmented, multi-step, and reasoning workflows where a model plans a step but the application validates every tool call before execution. The target competencies are GA-03.
...
A service desk workflow lets a model draft a ticket lookup, but the application validates the tool schema, confirms the user is allowed to see the ticket, and rejects unsafe arguments. The learner must identify the Azure component, validation gate, telemetry record, and remediation path before any live resource is used.
...
4. Reject one unsuitable option: Reject free-form tool arguments because malformed or injected values can call the wrong API.
...
- Keyword focus for review: tool schema validation, tool failure, multi-step, arguments, approval.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ga/GA-03.json"
}
```
