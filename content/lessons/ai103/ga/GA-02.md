---
{
  "schema_version": 1,
  "lesson_id": "GA-02",
  "title": "Implement retrieval-augmented generation",
  "competency_ids": [
    "GA-02"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/GA-02-rag.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ga/GA-02.json"
  ],
...
Scenario focus: Implement retrieval-augmented generation in an application that answers from approved organizational knowledge rather than from unsupported model memory. The target competencies are GA-02.
...
A benefits assistant uses Azure AI Search to retrieve policy chunks, sends only relevant passages to a Foundry model, and returns an answer with citations and a fallback when retrieval is weak. The learner must identify the Azure component, validation gate, telemetry record, and remediation path before any live resource is used.
...
4. Reject one unsuitable option: Reject a direct model call when the answer requires current private documents and citations.
...
- Keyword focus for review: retrieval, grounding, citation, chunk, index, fabrication.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ga/GA-02.json"
}
```
