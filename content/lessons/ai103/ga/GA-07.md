---
{
  "schema_version": 1,
  "lesson_id": "GA-07",
  "title": "Build grounded agents with retrieval, functions, and memory",
  "competency_ids": [
    "GA-08",
    "GA-09"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/GA-07-grounded-agent.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ga/GA-07.json"
  ],
...
# Build grounded agents with retrieval, functions, and memory
...
Scenario focus: Build agents that combine retrieval, function calling, conversation memory, APIs, knowledge stores, search, and content understanding without letting memory override policy. The target competencies are GA-08, GA-09.
...
A claims assistant retrieves policy text, calls a claim-status API through a validated function, uses short-term memory for conversation context, and redacts sensitive data in traces. The learner must identify the Azure component, validation gate, telemetry record, and remediation path before any live resource is used.
...
4. Reject one unsuitable option: Reject unbounded memory because stale or sensitive conversation state can contaminate later actions.
...
- Keyword focus for review: retrieval, function calling, memory, API, knowledge store, content understanding.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ga/GA-07.json"
}
```
