---
{
  "schema_version": 1,
  "lesson_id": "TA-04",
  "title": "Customize domain-specific extraction and summarization",
  "competency_ids": [
    "TA-04"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/TA-04-domain-extraction.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ta/TA-04.json"
  ],
...
# Customize domain-specific extraction and summarization
...
Scenario focus: customize language-model outputs for domain-specific extraction and summarization without using real clinical or customer records. The target competencies are TA-04.
...
A synthetic medical referral note is summarized into a domain-specific JSON handoff with required fields, excluded identifiers, and a deterministic rubric. The learner must document that examples are synthetic, choose between Foundry models and dedicated Foundry Tools, and record validation before live Azure use.
...
4. Reject one unsuitable option: Reject a generic summary when the workflow requires domain-specific fields and explicit exclusion of synthetic PII.
...
- Keyword focus for review: domain extraction, summarization, synthetic medical example, rubric, customization.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ta/TA-04.json"
}
```
