---
{
  "schema_version": 1,
  "lesson_id": "TA-06",
  "title": "Integrate speech and audio reasoning as agent modalities",
  "competency_ids": [
    "TA-06",
    "TA-07"
  ],
  ...
  "lab_links": [
    "scripts/labs/planned/TA-06-agent-audio.py"
  ],
  "assessment_links": [
    "content/assessments/ai103/ta/TA-06.json"
  ],
...
# Integrate speech and audio reasoning as agent modalities
...
Scenario focus: integrate speech as an agent modality, include custom speech models, and enable reasoning over audio inputs in multimodal flows. The target competencies are TA-06, TA-07.
...
An agent receives a synthetic voicemail fixture, transcribes it, reasons over the audio-derived content, and routes a request only after consent, accessibility, and confidence checks pass. The learner must document that examples are synthetic, choose between Foundry models and dedicated Foundry Tools, and record validation before live Azure use.
...
4. Reject one unsuitable option: Reject acting on low-confidence audio reasoning when transcript quality, speaker consent, or custom speech suitability is unclear.
...
- Keyword focus for review: agent modality, custom speech models, audio reasoning, confidence, consent.
...
```json
{
  ...
  "rubric_path": "content/assessments/ai103/ta/TA-06.json"
}
```
